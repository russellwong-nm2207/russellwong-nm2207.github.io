...
.then(function(data){
    ...
    for(i=0;i<table.length;i++)
    {
        ...
        if(table[i][3]<=-0.66)
        {
            negLabels.push(table[i][1]);
            negData.push(table[i][3]);
        }
        if(table[i][3]>=0.66)
        {
            posLabels.push(table[i][1]);
            posData.push(table[i][3]);
        }
    ...
    //positive chart
const dataObj2 = {
    labels: posLabels, //uncomment this and set this to negLabels
    datasets: [
        {
            label: "Words", // uncomment this line and set this to "Words"
            data: posData, // uncomment this line and set this to negData
            borderWidth: 2,
            backgroundColor: "hsla(20,100%,80%,0.8)",
            borderColor: "hsla(0,100%,50%,1)"
        }
...
new Chart("poswords-bar-chart",
{
    ...
    options: { 
        ...
        title: {
            display: true,
            text:['The most positive phrases in the Singlish vocabulary','According to research'], //set this to ['The most negative phrases in the Singlish vocabulary','According to research']
            fontFamily: "TrebuchetMS",
            fontSize: 24,
            fontColor: 'rgb(0,120,0)',
        }
